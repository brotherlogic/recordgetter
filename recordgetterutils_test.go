...
func TestGetPreFreshamanOnCategoryGet(t *testing.T) {
	...
	rec, err := s.getCategoryRecord(context.Background(), time.Now(), pbrc.ReleaseMetadata_PRE_FRESHMAN, &pb.State{}, pb.RequestType_DEFAULT)
	if err != nil {
...
func TestValidate(t *testing.T) {
	...
	valid := s.validate(&pbrc.Record{Release: &pbgd.Release{Formats: []*pbgd.Format{&pbgd.Format{Descriptions: []string{"7\""}}}}}, pb.RequestType_DEFAULT)
	if valid {
